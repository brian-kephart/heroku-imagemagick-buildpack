...
COMPILE_VERSION_LIBAOM=$(cat container/Dockerfile | egrep -o 'COMPILE_VERSION_LIBAOM=([0-9.-]+)' | cut -f 2 -d =)
...
if [ "$COMPILE_VERSION_LIBAOM" != "$LATEST_VERSION_LIBAOM" ]; then
  echo "[NEW UPDATE] COMPILE_VERSION_LIBAOM     $COMPILE_VERSION_LIBAOM 🔄 $LATEST_VERSION_LIBAOM"
fi
